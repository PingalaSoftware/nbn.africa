...
let useCaseCarouselInnerWidth = useCaseCarouselInner.getBoundingClientRect().width;
...
const useCaseTotalMovementSize =
    parseFloat(
        document.querySelector(".useCaseCarousel-item").getBoundingClientRect().width,
        10
    ) +
    parseFloat(
...
useCaseNext.addEventListener("click", () => {
    const useCaseCarouselVpWidth = useCaseCarouselVp.getBoundingClientRect().width;
    if (carouselInnerWidth - Math.abs(useCaseLeftValue) > useCaseCarouselVpWidth) {
        useCaseLeftValue -= useCaseTotalMovementSize;
        useCaseCarouselInner.style.left = useCaseLeftValue + "px";
    }
});
...
function mediaManagement() {
    ...
    if (useCasePrev <= -useCaseTotalMovementSize && useCaseOldViewportWidth < useCaseNewViewportWidth) {
